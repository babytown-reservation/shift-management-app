...
- Supabase Auth
  - 管理者: メールアドレス + パスワードでログイン
  - スタッフ: メールアドレスのみでマジックリンクログイン
...
管理者でログインすると管理者画面が表示されます。スタッフは管理者画面のスタッフ管理から、スタッフ名とメールアドレスを入力して招待します。招待に成功すると、Supabase Authユーザー、`staff.email`、`staff.auth_user_id` が紐づきます。

## ログイン方式

### 管理者

管理者はメールアドレス + パスワードでログインします。ログイン画面で「管理者」を選択してください。パスワード再設定メール送信も管理者用導線として残しています。

### スタッフ

スタッフはメールアドレスのみでログインします。ログイン画面で「スタッフ」を選択し、メールアドレスを入力すると、Supabase Auth の `signInWithOtp` によってログインリンクが送信されます。

スタッフログインは `staff.email` に登録済みのメールアドレスだけ許可します。未登録メールアドレスの場合はエラーになります。メール内リンクを押すとログインが完了し、自分に紐づく希望休と確定シフトだけを閲覧できます。
...
6. `Deploy` を実行します。
7. デプロイ完了後、VercelのFunctionsに `/api/staff/invite` と `/api/auth/staff-magic-link` が含まれることを確認します。

## デプロイ後の動作確認チェックリスト

- 未ログイン状態でアクセスするとログイン画面が表示される。
- 管理者ユーザーでメールアドレス + パスワードログインできる。
- 管理者ログイン後、管理者画面が表示される。
- 管理者パスワード再設定メールを送信できる。
- 管理者画面でスタッフ名とメールアドレスを入力して招待できる。
- 招待したスタッフの `staff.email` と `staff.auth_user_id` がSupabaseで設定されている。
- スタッフログインでメールアドレスのみを入力し、ログインリンクを送信できる。
- 未登録メールアドレスではスタッフログインリンクを送信できない。
- スタッフがメール内リンクを押すとログイン完了する。
...
- スタッフ: `staff.auth_user_id = auth.uid()` または `staff.email = Authユーザーのemail` に一致する自分のスタッフ行だけ読み取れます。
