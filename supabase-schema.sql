...
on public.time_off_requests for all
using (exists (select 1 from public.staff s where s.id = staff_id and s.auth_user_id = auth.uid()))
with check (exists (select 1 from public.staff s where s.id = staff_id and s.auth_user_id = auth.uid()));
